...
public class ShellyEM3MsgHandler implements MsgHandler {
    ...
    public void handelMsg(BrokerMsgEnity brokerMsgEnity) {
        ...
        if (brokerMsgEnity.getTopic().contains("emeter")) {
           setEmeterValue(brokerMsgEnity);
            ...
            ((ShellyEM3Entity)this.SHELLYS_AND_CHANELS.getDevice(brokerMsgEnity.getClientID())).getRelayList().get(Integer.parseInt(brokerMsgEnity.getTopic().split("/")[3])).setStatus(brokerMsgEnity.getMsg());
    ...
    private void setEmeterValue(BrokerMsgEnity brokerMsgEnity){
        ...
        EmeterEntity emeter =  ((ShellyEM3Entity) this.SHELLYS_AND_CHANELS.getDevice(brokerMsgEnity.getClientID())).getEmeterList().get(emeterIndex);

        if (brokerMsgEnity.getTopic().contains("total")) {
            emeter.setTotal(Float.parseFloat(brokerMsgEnity.getMsg()));
